...
@Component({
  selector: 'app-users-job-data',
  templateUrl: './users-job-data.component.html',
  styleUrls: ['./users-job-data.component.css']
})
export class UsersJobDataComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.auth.getUsersJobDetails(this.usrTenantId).subscribe((res) => {
      console.log(res);
      this.WorkData = res.jobData;
    ...
    if (clickedYes) {
      ...
      this.auth.UpdateUserJobData(this.editForm1.value)
        .subscribe({
          next: (res => {
            alert(res.message);
            this.router.navigate(['usersJobData']);
          }),
          error: (err => {
